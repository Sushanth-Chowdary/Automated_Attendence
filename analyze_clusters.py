...
# --- THE SUPER PALETTE ---
# 1. Grab all the best contrasting color sets from Matplotlib
...
colors_set2 = plt.cm.Set2.colors
colors_set3 = plt.cm.Set3.colors

# 2. Combine them into one massive list of 89 distinct colors
combined_colors = list(colors_tab20) + list(colors_tab20b) + list(colors_tab20c) + \
                  list(colors_set1) + list(colors_set2) + list(colors_set3)

# 3. Remove duplicates just in case some palettes share exact hex codes
unique_colors = list(dict.fromkeys(combined_colors))

# 4. Take only as many as we need for our students
full_palette = unique_colors[:len(labels_unique)]

# 5. Shuffle them so similar shades don't end up next to each other in the legend!
random.seed(42) 
...
scatter = sns.scatterplot(
    ...
    data=plot_df,
    legend="full",
    alpha=0.8,   # Made dots slightly more opaque
    s=70         # Made dots slightly larger
)

plt.title("Face Embeddings Visualization (t-SNE Map)", fontsize=18, fontweight='bold')
plt.xlabel("Dimension 1", fontsize=12)
plt.ylabel("Dimension 2", fontsize=12)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize=8, markerscale=1.5)
plt.tight_layout()
